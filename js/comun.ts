 ...
 */
export function iniciarPie(
	nodo_padre:			HTMLDivElement, 
	texto_nivel:		string, 
	texto_titulo:		string, 
 ...
 * @return {void} Esta función no devuelve ningún valor.
 */
export function modificarTextoPorId(
	id: 	string,
	texto : string
) : void {

	const contenedor = document.getElementById(id) 
	if(contenedor !== null) contenedor.innerHTML = texto; else throw new Error("No existe el elemento (ID:"+id+")");
}
/**
 ...
 */
export function crearNodoTexto(
	nombre_nodo:	string, 
	texto: 			string, 
	id?: 			string 
) : HTMLElement {
 ...
 * @return {void} Esta función no devuelve ningún valor.
 */
export function modificarFavicon(
	archivo : string
) : void {

	const link = document.querySelector("head>link[rel='icon']");
	if(link !== null) link.setAttribute("href", archivo); else throw new Error("No existe favicon");
}
/**
 ...
 */
export async function insertarSVG(
	id_contenedor: string, 
	url_SVG: string
) : Promise<void> {
	
	const contenedor = document.getElementById(id_contenedor);
	if(contenedor === null)	throw new Error("No existe el elemento (ID:"+id_contenedor+")");
